...
# 5) Added Integral Table dialog for viewing all integrals in a copyable table.
...
def compute_integral(fid, time, lb, user_phase_offset, ppm_start, ppm_stop):
    ...
    This is extracted from process_and_plot() to avoid code duplication.
    """
    ...
    # Sort by ascending ppm for integration
...
class CopyableTableWidget(QTableWidget):
    """
    A QTableWidget subclass that supports Ctrl+C to copy selected cells
    in a tab-delimited format suitable for pasting into Excel.
    """
    ...
    def copy_selection(self):
        """Copy selected cells to clipboard as tab-delimited text."""
...
class IntegralTableDialog(QDialog):
    """
    A dialog that displays a table of integrals from analyzed spectra.
    Columns: File Name, Transient, Integral
    """
    def __init__(self, parent=None):
        ...
        # Make the dialog non-modal so the user can interact with the main window
        self.setModal(False)
        ...
        # Enable selection of individual cells and multiple cells
        ...
        # Stretch columns to fill the dialog
    ...
    def populate(self, data, phase_offsets, lb, ppm_start, ppm_stop):
        """
        Populate the table with integral data.
        
        Args:
            data: List of tuples (fid, time, folder_name, transient_num)
            phase_offsets: List of phase offsets corresponding to data
            lb: Line broadening value
            ppm_start: PPM window start
            ppm_stop: PPM window stop
        """
        ...
        for i, (fid, time, folder_name, transient_num) in enumerate(data):
            ...
            # File Name
            file_item = QTableWidgetItem(folder_name)
            self.table.setItem(i, 0, file_item)
            
            # Transient
            trans_item = QTableWidgetItem(str(transient_num))
            self.table.setItem(i, 1, trans_item)
            
            # Integral (3 decimals, no comma separator)
            integral_item = QTableWidgetItem(f"{integral:.3f}")
            self.table.setItem(i, 2, integral_item)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.integral_dialog = None # reference to the integral table dialog
        ...
        # Integral Table button (below phase controls, above plot)
        ...
        integral_btn_row.addStretch(1)  # Push button to the left
    ...
    def _close_integral_dialog(self):
        """Close the integral table dialog if it's open."""
    ...
    def _refresh_integral_dialog(self):
        """Refresh the integral table dialog if it's open."""
        if self.integral_dialog is not None and self.integral_dialog.isVisible():
            lb = float(self.lb_spin.value())
            ppm_start = float(self.ppm_start.value())
            ppm_stop = float(self.ppm_stop.value())
            self.integral_dialog.populate(self.data, self.phase_offsets, lb, ppm_start, ppm_stop)

    def on_show_integral_table(self):
        """Show the integral table dialog."""
        if not self.data:
    ...
    def on_run(self):
        # Close the integral dialog if it's open (per user spec: close but don't reopen)
