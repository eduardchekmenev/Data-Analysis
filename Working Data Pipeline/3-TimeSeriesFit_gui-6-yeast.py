...
class BicarbonateFitConfig:
    ...
    T1CO_bounds_s: Tuple[float, float] = (10.0, 100.0)
    T1BC_bounds_s: Tuple[float, float] = (10.0, 100.0)
...
def integrate_co2_bicarbonate(params, t_eval, P_interp, tinj=0.0):
    """
    Integrate the CO₂/bicarbonate ODE system.
    
    params = [ksubCO, kCOBC, kBCCO, R_CO2, R_BC]
    tinj = injection time offset (integration starts from -tinj to allow pre-accumulation)
    
    Returns interpolated CO2 and BIC values at t_eval.
    """
    # Start integration from -tinj to allow metabolite accumulation before t=0
    t_start = -tinj
    t_end = t_eval[-1]
    
    # Dense integration grid from -tinj to t_end
    t_dense = np.linspace(t_start, t_end, 500)
    
    sol = solve_ivp(
        lambda t, y: ode_co2_bicarbonate(t, y, params, P_interp),
        t_span=(t_start, t_end),
        y0=[0.0, 0.0],  # Initial CO2 and BIC are zero at t=-tinj
        t_eval=t_dense,
        method='RK45'
    )
...
def _build_residuals(theta, tP, Pobs, tCO2, CO2obs, tBC, BCobs, cfg: BicarbonateFitConfig, tinj: float):
    ...
    # Must cover from -tinj (where ODE integration starts) to max data time
    t_all = np.unique(np.concatenate([tP, tCO2, tBC]))
    t_interp_start = -tinj - 1.0  # Start slightly before -tinj
    t_interp_end = max(t_all.max(), tP.max()) + 10
    t_dense_for_interp = np.linspace(t_interp_start, t_interp_end, 500)
    ...
    # Compute CO2 and bicarbonate models (ODE integration starting from -tinj)
    ode_params = [ksubCO, kCOBC, kBCCO, R_CO2, R_BC]
    
    # Evaluate at CO2 time points
    CO2mod, _ = integrate_co2_bicarbonate(ode_params, tCO2, P_interp, tinj)
    
    # Evaluate at bicarbonate time points
    _, BCmod = integrate_co2_bicarbonate(ode_params, tBC, P_interp, tinj)
...
def run_fit_on_file(
    ...
    *,
    ...
    # Create pyruvate interpolator for ODE
    # Must cover from -tinj (where ODE integration starts) to max time
    t_interp_start = -fit.tinj - 1.0
    t_interp_end = t_max + 10
    t_interp_grid = np.linspace(t_interp_start, t_interp_end, 500)
    ...
    # Integrate CO2 and bicarbonate on full dense grid (ODE starts from -tinj)
    CO2mod_dense, BCmod_dense = integrate_co2_bicarbonate(ode_params, t_dense, P_interp, fit.tinj)
    ...
    # Create pyruvate interpolator for residual ODE computation
    # Must cover from -tinj to max data time
    t_data_max = max(tCO2.max() if tCO2.size else 0, tBC.max() if tBC.size else 0, tP.max() if tP.size else 0)
    t_interp_full = np.linspace(-fit.tinj - 1.0, t_data_max + 10, 500)
    ...
    # Evaluate model at data time points (ODE starts from -tinj)
    CO2mod_data, _ = integrate_co2_bicarbonate(ode_params, tCO2, P_interp_full, fit.tinj)
    _, BCmod_data = integrate_co2_bicarbonate(ode_params, tBC, P_interp_full, fit.tinj)
...
class ParametersDialog(QDialog):
    def __init__(self, parent, cfg: BicarbonateFitConfig):
        ...
        # Injection time (tinj)
        tinj_row = QHBoxLayout()
        self.sb_tinj = QDoubleSpinBox()
        self.sb_tinj.setDecimals(2)
        self.sb_tinj.setRange(0, 100)
        self.sb_tinj.setSingleStep(1.0)
        self.sb_tinj.setValue(cfg.fixed_tinj_value)
        self.cb_fit_tinj = QCheckBox("Fit tinj")
        self.cb_fit_tinj.setChecked(cfg.fit_tinj)
        tinj_row.addWidget(self.sb_tinj)
        tinj_row.addWidget(self.cb_fit_tinj)
        holder_tinj = QWidget()
        holder_tinj.setLayout(tinj_row)
        form.addRow(QLabel("Injection time tinj (s):"), holder_tinj)
    ...
    def on_save(self):
        ...
        self.cfg.fixed_tinj_value = self.sb_tinj.value()
        self.cfg.fit_tinj = self.cb_fit_tinj.isChecked()
...
class FitGUI(QWidget):
    def __init__(self):
        ...
        self.cb_csv_summary.setChecked(False)
