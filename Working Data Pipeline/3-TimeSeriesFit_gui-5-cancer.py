...
@dataclass
class TwoSiteConfig:
    allow_curve_scales: bool = True
    kpl_bounds: Tuple[float, float] = (0.00001, 0.05)
    Rp_eff_bounds: Tuple[float, float] = (0.015, 0.07)
    Rl_bounds: Tuple[float, float] = (0.01, 0.2)
    tinj_extra_hi: float = 10.0
    fit_tinj: bool = False
    fixed_tinj_value: float = 8.0
    sP_bounds: Tuple[float, float] = (1.7, 2.3)
    sL_bounds: Tuple[float, float] = (0.5, 0.75)
...
# ===== Joint residual builder (unchanged) =====
...
def fit_two_site_joint(tP_raw, Pobs_raw, tL_raw, Lobs_raw, cfg: TwoSiteConfig) -> FitResult:
    ...
    # ---- Δ bounds from T1p limits ----
    T1p_lo, T1p_hi = cfg.T1p_bounds_s
    delta_lo = 1.0 / float(T1p_hi)
    delta_hi = 1.0 / float(T1p_lo)
    ...
    kpl0, Rl0 = 0.01, 0.02
    sP0, sL0 = 1.0, 1.0
    # ensure scales start inside bounds
    sP0 = float(np.clip(sP0, cfg.sP_bounds[0], cfg.sP_bounds[1]))
    sL0 = float(np.clip(sL0, cfg.sL_bounds[0], cfg.sL_bounds[1]))
    # derive Δ initial guess within T1p bounds
    Delta0 = float(np.clip(Rp0 - kpl0, delta_lo, delta_hi))


    # ---- build θ ----
    if cfg.allow_curve_scales:
        if cfg.fit_tinj:
            x0 = np.array([P0_0, kpl0, Delta0, Rl0, sP0, sL0, tinj0_guess])
            lb = np.array([0.0, cfg.kpl_bounds[0], delta_lo, cfg.Rl_bounds[0],
                           cfg.sP_bounds[0], cfg.sL_bounds[0], tinj_lo])
            ub = np.array([np.inf, cfg.kpl_bounds[1], delta_hi, cfg.Rl_bounds[1],
                           cfg.sP_bounds[1], cfg.sL_bounds[1], tinj_hi])

            def fun_fit(theta):
                P0_, kpl_, Delta_, Rl_, sP_, sL_, tinj_ = theta
                Rp_eff_ = kpl_ + Delta_
                return _build_residuals([P0_, kpl_, Rp_eff_, Rl_, sP_, sL_],
                                        tP, Pobs_raw, tL, Lobs_raw, cfg, tinj_)

            res = least_squares(fun_fit, x0, bounds=(lb, ub), method='trf', max_nfev=5000)
            P0, kpl, Delta, Rl, sP, sL, tinj = res.x
        else:
            x0 = np.array([P0_0, kpl0, Delta0, Rl0, sP0, sL0])
            lb = np.array([0.0, cfg.kpl_bounds[0], delta_lo, cfg.Rl_bounds[0],
                           cfg.sP_bounds[0], cfg.sL_bounds[0]])
            ub = np.array([np.inf, cfg.kpl_bounds[1], delta_hi, cfg.Rl_bounds[1],
                           cfg.sP_bounds[1], cfg.sL_bounds[1]])

            def fun_fixed(theta):
                P0_, kpl_, Delta_, Rl_, sP_, sL_ = theta
                Rp_eff_ = kpl_ + Delta_
                return _build_residuals([P0_, kpl_, Rp_eff_, Rl_, sP_, sL_],
                                        tP, Pobs_raw, tL, Lobs_raw, cfg, tinj_for_residuals)

            res = least_squares(fun_fixed, x0, bounds=(lb, ub), method='trf', max_nfev=5000)
            P0, kpl, Delta, Rl, sP, sL = res.x
            tinj = tinj_for_residuals
    else:
        if cfg.fit_tinj:
            x0 = np.array([P0_0, kpl0, Delta0, Rl0, tinj0_guess])
            lb = np.array([0.0, cfg.kpl_bounds[0], delta_lo, cfg.Rl_bounds[0], tinj_lo])
            ub = np.array([np.inf, cfg.kpl_bounds[1], delta_hi, cfg.Rl_bounds[1], tinj_hi])

            def fun_fit(theta):
                P0_, kpl_, Delta_, Rl_, tinj_ = theta
                Rp_eff_ = kpl_ + Delta_
                return _build_residuals([P0_, kpl_, Rp_eff_, Rl_],
                                        tP, Pobs_raw, tL, Lobs_raw, cfg, tinj_)

            res = least_squares(fun_fit, x0, bounds=(lb, ub), method='trf', max_nfev=5000)
            P0, kpl, Delta, Rl, tinj = res.x
            sP, sL = 1.0, 1.0
        else:
            x0 = np.array([P0_0, kpl0, Delta0, Rl0])
            lb = np.array([0.0, cfg.kpl_bounds[0], delta_lo, cfg.Rl_bounds[0]])
            ub = np.array([np.inf, cfg.kpl_bounds[1], delta_hi, cfg.Rl_bounds[1]])

            def fun_fixed(theta):
                P0_, kpl_, Delta_, Rl_ = theta
                Rp_eff_ = kpl_ + Delta_
                return _build_residuals([P0_, kpl_, Rp_eff_, Rl_],
                                        tP, Pobs_raw, tL, Lobs_raw, cfg, tinj_for_residuals)

            res = least_squares(fun_fixed, x0, bounds=(lb, ub), method='trf', max_nfev=5000)
            P0, kpl, Delta, Rl = res.x
            sP, sL = 1.0, 1.0
            tinj = tinj_for_residuals
...
class ParametersDialog(QDialog):
    def __init__(self, parent, cfg: TwoSiteConfig):
        ...
        # Helper to make a min/max spinbox pair
        def mk_pair(lo, hi, step=0.01):
            w_lo = QDoubleSpinBox(); w_hi = QDoubleSpinBox()
            w_lo.setDecimals(6); w_hi.setDecimals(6)
            w_lo.setRange(-1e9, 1e9); w_hi.setRange(-1e9, 1e9)
            w_lo.setSingleStep(step); w_hi.setSingleStep(step)
            w_lo.setValue(lo); w_hi.setValue(hi)
            row = QHBoxLayout(); row.addWidget(w_lo); row.addWidget(QLabel("to")); row.addWidget(w_hi)
            return row, w_lo, w_hi

        # kpl bounds
        row, self.kpl_lo, self.kpl_hi = mk_pair(cfg.kpl_bounds[0], cfg.kpl_bounds[1], 0.00001)
        holder = QWidget(); holder.setLayout(row)
        form.addRow(QLabel("kpl bounds:"), holder)

        # sP bounds
        row, self.sP_lo, self.sP_hi = mk_pair(cfg.sP_bounds[0], cfg.sP_bounds[1], 0.01)
        holder = QWidget(); holder.setLayout(row)
        form.addRow(QLabel("sP bounds:"), holder)

        # sL bounds
        row, self.sL_lo, self.sL_hi = mk_pair(cfg.sL_bounds[0], cfg.sL_bounds[1], 0.01)
        holder = QWidget(); holder.setLayout(row)
        form.addRow(QLabel("sL bounds:"), holder)

        # T1p bounds (seconds)
        row, self.T1p_lo, self.T1p_hi = mk_pair(cfg.T1p_bounds_s[0], cfg.T1p_bounds_s[1], 0.1)
        holder = QWidget(); holder.setLayout(row)
        form.addRow(QLabel("T1p bounds (s):"), holder)

        # AUC window (seconds)
        self.auc_min = QDoubleSpinBox(); self.auc_max = QDoubleSpinBox()
        ...
        row_auc = QHBoxLayout()
        row_auc.addWidget(self.auc_min); row_auc.addWidget(QLabel("to")); row_auc.addWidget(self.auc_max)
        holder_auc = QWidget(); holder_auc.setLayout(row_auc)
    ...
    def on_save(self):
        # Validate min<=max
        pairs = [(self.kpl_lo.value(), self.kpl_hi.value(), "kpl"),
                 (self.sP_lo.value(), self.sP_hi.value(), "sP"),
                 (self.sL_lo.value(), self.sL_hi.value(), "sL"),
                 (self.T1p_lo.value(), self.T1p_hi.value(), "T1p"),
                 (self.auc_min.value(), self.auc_max.value(), "AUC window (s)")]  # <-- NEW]
        ...
        # Apply to cfg
        self.cfg.kpl_bounds = (self.kpl_lo.value(), self.kpl_hi.value())
        self.cfg.sP_bounds = (self.sP_lo.value(), self.sP_hi.value())
        self.cfg.sL_bounds = (self.sL_lo.value(), self.sL_hi.value())
...
def two_site_config_to_dict(cfg: TwoSiteConfig) -> dict:
    return {
        "allow_curve_scales": cfg.allow_curve_scales,
        "kpl_bounds": cfg.kpl_bounds,
        "Rp_eff_bounds": cfg.Rp_eff_bounds,
        "Rl_bounds": cfg.Rl_bounds,
        "tinj_extra_hi": cfg.tinj_extra_hi,
        "fit_tinj": cfg.fit_tinj,
        "fixed_tinj_value": cfg.fixed_tinj_value,
        "sP_bounds": cfg.sP_bounds,
        "sL_bounds": cfg.sL_bounds,
        "flip_angle_deg": cfg.flip_angle_deg,
        "TR_s": cfg.TR_s,
...
def _result_row_from_payload(payload):
    ...
    # --- AUC of fitted Lactate curve over the configured window [CFG.auc_tmin_s, CFG.auc_tmax_s] ---
    t_dense = payload["t_dense"]
    L_dense = payload["Lmod_dense"]

    # Clip to window; if no points, AUC = 0.0
    w = (t_dense >= CFG.auc_tmin_s) & (t_dense <= CFG.auc_tmax_s)
    if np.any(w):
        auc_l = float(np.trapezoid(L_dense[w], t_dense[w]))
    else:
        # If the window lies entirely between two grid points, do a tiny linear interpolation fallback
        # so AUC isn't silently zero due to coarse grid; otherwise keep zero.
        try:
            t0, t1 = CFG.auc_tmin_s, CFG.auc_tmax_s
            # linear interp on edges
            L0 = np.interp(t0, t_dense, L_dense)
            L1 = np.interp(t1, t_dense, L_dense)
            auc_l = float(0.5 * (L0 + L1) * (t1 - t0))
        except Exception:
            auc_l = 0.0

    row["AUC_l"] = auc_l  # keep as last column
...
class FitGUI(QWidget):
    ...
    def on_open_params(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            cfgd = two_site_config_to_dict(CFG)
            self.output_tab.append(f"[Parameters] Updated: "
                                   f"sP={cfgd['sP_bounds']}, sL={cfgd['sL_bounds']}, "
                                   f"kpl={cfgd['kpl_bounds']}, T1p_s={cfgd['T1p_bounds_s']}, "
                                   f"AUC_window_s=({cfgd['auc_tmin_s']}, {cfgd['auc_tmax_s']})")
    ...
    def on_run_fit(self):
        ...
        # Always write fitparameters if any export option is selected
        if any([do_png, do_pdf, do_csv_each, do_csv_summary]) and self.output_folder is not None:
            first_file = selected[0] if selected else ""
            exptdate = extract_exptdate(os.path.basename(first_file)) or ""
            datepart = f"{exptdate}_" if exptdate else ""
            fitparam_path = self.output_folder / f"fitparameters_{datepart}fit_at{datetime.now().strftime('%Y%m%d-%H%M')}.txt"
            try:
                with open(fitparam_path, 'w', encoding='utf-8') as fp:
                    for k, v in two_site_config_to_dict(CFG).items():
                        fp.write(f"{k} = {v}\n")
                self.output_tab.append(f"Fit parameters saved to {fitparam_path}")
            except Exception as e:
                self.output_tab.append(f"ERROR writing fit parameters: {e}")

        # If no summary was requested, still write fitparameters when exporting anything
        if not do_csv_summary and any([do_png, do_pdf, do_csv_each]) and self.output_folder is not None:
            first_file = selected[0] if selected else ""
            exptdate = extract_exptdate(os.path.basename(first_file)) or ""
            datepart = f"{exptdate}_" if exptdate else ""
            fitparam_path = self.output_folder / f"fitparameters_{datepart}fit_at{datetime.now().strftime('%Y%m%d-%H%M')}.txt"
