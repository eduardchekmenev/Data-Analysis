...
class DataStatsGUI(QWidget):
    ...
    def run_stats(self):
        ...
        # === Draw plots ===
        self.draw_boxplots(df, variables)
        self.draw_pca(df, variables)
    
    def draw_boxplots(self, df, variables):
        ...
        for i, var in enumerate(variables):
            ...
            sns.boxplot(
                ...
                palette=self.color_map, order=sorted(sub_df["Group"].unique())
            )
            sns.stripplot(
                x="Group", y="Value", data=sub_df, ax=ax,
                jitter=True, marker="o", edgecolor="gray", linewidth=0.5,
                palette=self.color_map, alpha=0.6,
                order=sorted(sub_df["Group"].unique())
            )
